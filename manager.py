...
def is_container_running(real_name: str) -> bool:
    ...
    try:
        ps = subprocess.run(
            ["docker", "ps", "--filter", f"nombre={real_name}", "--format", "{{.ID}}"],
...
def check_latest(short: str):
    ...
    # Simple placeholder – in a real implementation we would compare image digests/tags
    log_info(short, "Check performed (placeholder)")
    log_entry(short, "Checked if container is latest (no-op)")
